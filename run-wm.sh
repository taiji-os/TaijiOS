#!/bin/sh
# TaijiOS build and run script with window manager auto-start
...
echo "=== TaijiOS Build & Run Script (Window Manager) ==="
echo "Detected OS: $OS"
echo ""
...
run_nixos() {
    echo "Starting emu with window manager on NixOS..."
    ...
    # Run emu with nix-shell environment, passing wm/wm.dis as the program to run
    exec nix-shell --run "
        export PATH='$ROOT/Linux/amd64/bin:\$PATH'
        export ROOT='$ROOT'
        cd '$ROOT'
        exec Linux/amd64/bin/emu -r '$ROOT' /dis/wm/wm.dis
    "
}

# Function to run on OpenBSD
run_openbsd() {
    echo "Starting emu with window manager on OpenBSD..."
    echo "Type 'exit' to quit emu"
    echo ""
    ...
    # Run emu with window manager
    exec "$ROOT/Linux/amd64/bin/emu" -r "$ROOT" /dis/wm/wm.dis
}

# Function to run on generic Linux
run_linux() {
    echo "Starting emu with window manager..."
    echo "Type 'exit' to quit emu"
    echo ""
