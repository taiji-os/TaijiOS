...
new(top: ref Tk->Toplevel, buts: int): chan of string
{
	...
	# Build title bar - NO color specifications, uses tk->env defaults
	# Kernel theme system handles all colors automatically
	cmd(top, "frame .Wm_t -borderwidth 1");
	cmd(top, "label .Wm_t.title -anchor w");
